...
class ContrastiveReconstructImagingAndTabularHeaderDataset(Dataset):
    """
    ...
    def __init__(
        ...
        # self.header = torch.load(field_lengths_tabular.replace('tabular_length.pt', 'biobert_header.pt'))
        print("Loading header from: ", field_lengths_tabular.replace('tabular_length.pt', 'bert_header.pt'))
        self.header = torch.load(field_lengths_tabular.replace('tabular_length.pt', 'bert_header.pt'))
    ...
    def read_and_parse_csv(self, path_tabular: str) -> List[List[float]]:
        ...
        for arr in data:
            current_len = len(arr)
            padded_array = np.pad(arr, (0, len(self.field_lengths_tabular) - current_len), mode='constant',
                                  constant_values=0)
    ...
    def generate_imaging_views(self, index: int) -> List[torch.Tensor]:
        ...
        if self.live_loading:
            # im = im.replace('/ADNI/', '/ADNI_npy/').replace('/AIBL/', '/AIBL_npy/').replace('/NACC/', '/NACC_npy/').replace('.nii.gz', '.npy')
    ...
    def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        if self.c > 0:
            tabular_views = [
                torch.tensor(self.corrupt(self.data_tabular[index], np.sum(self.valid_mask[index])), dtype=torch.float)]
        else:
        ...
        unaugmented_tabular = torch.tensor(self.data_tabular[index],
                                           dtype=torch.float)  # tabular views: corrupt, raw, mask, mask_specical

        len_mask = self.valid_mask[index] != 1

        tabular_views = tabular_views + [torch.tensor(len_mask, dtype=torch.bool)]

        # print(self.valid_mask[index], np.sum(self.valid_mask[index]))
        if np.sum(self.valid_mask[index]) == 14:
            header = self.header['adni']
        elif np.sum(self.valid_mask[index]) == 7:
            header = self.header['aibl']
        elif np.sum(self.valid_mask[index]) == 21:
            header = self.header['nacc']

        header_embedding = torch.zeros(21, 768)
        header_embedding[:header.shape[0], :] = header

        return imaging_views, tabular_views, label, unaugmented_image, unaugmented_tabular, header_embedding
