...
class ImagingAndTabularDataset(Dataset):
  ...
  def __init__(
    ...
    # print(field_lengths_tabular.replace('tabular_length.pt', 'bert_header.pt'))
  ...
  def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor]:
    ...
    if self.missing_tabular:
      ...
      return (im, tab, missing_mask, path, self.header), label
    else:
      return (im, tab, path, self.header), label

  def __len__(self) -> int:
    return len(self.data_tabular)


# class ImagingAndTabularMergeDataset(Dataset):
#   """
#   Multimodal dataset that imaging and tabular data for evaluation.
#   Load mask csv to imitate missing tabular data
#   missing_strategy: value or feature
...
#     # Tabular
#     self.field_lengths_tabular = np.array(torch.load(field_lengths_tabular))
#     self.data_tabular = np.array(self.read_and_parse_csv(data_path_tabular)[0])
#     self.tabular_mask = np.array(self.read_and_parse_csv(data_path_tabular)[1])
#     self.generate_marginal_distributions()
#     self.eval_one_hot = eval_one_hot
#     self.c = corruption_rate if corruption_rate else None
#
...
#       else:
#         out.append(
#           torch.nn.functional.one_hot(torch.clamp(subject[i], min=0, max=self.field_lengths_tabular[i] - 1).long(),
...
#
#     label = torch.tensor(self.labels[index], dtype=torch.long)
#     tab_mask = ~torch.tensor(self.tabular_mask[index], dtype=torch.bool)
#
#     print(self.tabular_mask[index])
#     if np.sum(self.tabular_mask[index]) == 14:
#       header = self.header['adni']
#     elif np.sum(self.tabular_mask[index]) == 7:
#       header = self.header['aibl']
#     elif np.sum(self.tabular_mask[index]) == 27:
#       header = self.header['adni']
#
#     header_embedding = torch.zeros(27, 768)
#     header_embedding[:header.shape[0], :] = header
#
#     if self.missing_tabular:
#       missing_mask = torch.from_numpy(self.missing_mask_data[index])
#       return (im, tab, missing_mask, path, tab_mask, header_embedding), label
#     else:
#       return (im, tab, path, tab_mask, header_embedding), label
#
#   def __len__(self) -> int:
#     return len(self.data_tabular)
