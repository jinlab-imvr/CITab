...
class Pretraining(pl.LightningModule):
  ...
  def initialize_imaging_encoder_and_projector(self) -> None:
    ...
    if self.hparams.model == 'resnet50':
      self.encoder_imaging = nn.Sequential(*list(resnet50().children())[:-1])
    elif self.hparams.model == 'resnet18':
      self.encoder_imaging = nn.Sequential(*list(resnet18().children())[:-1])
  ...
  def forward_imaging(self, x: torch.Tensor) -> torch.Tensor:
    ...
    y = self.encoder_imaging(x).squeeze()
