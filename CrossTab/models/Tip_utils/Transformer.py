...
class TabularTransformerEncoder(nn.Module):
    ...
    def __init__(self, args: Dict) -> None:
        ...
        args.share_num = 5
        print(args.share_num)
        self.transformer_blocks = nn.ModuleList([
            MoE_Block(dim=args.tabular_embedding_dim, drop=args.drop_rate, share_num=args.share_num, is_cross_attention=False)
...
class MultimodalTransformerEncoder(nn.Module):
    ...
    def __init__(self, args: Dict) -> None:
        ...
        self.transformer_blocks = nn.ModuleList([
            Block(dim=args.multimodal_embedding_dim, is_cross_attention=False,
    ...
    def forward(self, x: torch.Tensor, image_features: torch.Tensor, mask=None, visualize=False) -> torch.Tensor:

        if len(image_features.shape) == 5:
            B,C,D,H,W = image_features.shape
            image_features = image_features.reshape(B,C,D*H*W).permute(0,2,1)

        image_features = self.image_proj(image_features)
        image_features = self.image_norm(image_features)
        # torch.save(x[:, 1:, :], './tsne_fea_nacc/batch_%d.pt'%(len(os.listdir('./tsne_fea_nacc'))))
        # print(x[:, 1:, :].shape)
        ...
        if visualize == False:
            if mask is not None:
                B, N = mask.shape
                cls_mask = torch.zeros(B, 1).bool().to(mask.device)
                img_mask = torch.zeros(B, D*H*W).bool().to(mask.device)
                mask_init = torch.cat([cls_mask, mask, img_mask], dim=1)
                mask = mask_init[:, None, :].repeat(1, N + D*H*W + 1, 1)
                mask_eye = ~torch.eye(N + D*H*W + 1).bool().to(mask.device)
                mask_eye = mask_eye[None, :, :]
                mask = mask * mask_eye
                mask = mask[:, None, :, :]
                mask = mask * (-1e9)

                # mask_cross = mask_init[:, None, :, None]  # 变成 (B,1,N,1)
                # mask_cross = mask_cross.repeat(1, 1, 1, image_features.shape[1])
                # mask_cross = mask_cross * (-1e9)
