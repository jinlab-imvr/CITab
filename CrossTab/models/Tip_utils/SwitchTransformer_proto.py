...
class SwitchGate(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        # gate_indices = torch.argmax(gate_scores, dim=-1)
        # with open('/mnt/zt/fyb/BrainMRI-CrossTab-Pretraining/result/runs/nacc_gate_score.txt', "a") as f:
        #     for row in gate_indices.cpu().numpy():
        #         formatted_row = " ".join(f"{x:.6f}" for x in row)  # 格式化为 6 位小数
...
class SwitchMoE(nn.Module):
    ...
    def __init__(
        ...
        self.expert_centroids = expert_centroids
       
        self.selective_experts = nn.ModuleList(
            [
                Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
                for _ in range(num_experts - self.share_num)
            ]
        )
        # self.gate = SwitchGate(dim, num_experts - share_num,  capacity_factor)
        # self.expert_centroids = nn.Parameter(
        #     torch.randn(num_experts - share_num, dim)  # [num_routed_experts, d_model]
        # )
...
class MoE_Block(nn.Module):
    def __init__(self, dim, expert_centroids, num_heads=8, is_cross_attention=False, encoder_dim=None, share_num=0, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = SwitchMoE(dim=dim, expert_centroids=expert_centroids, mlp_hidden_dim=int(dim * 1.75), act_layer=act_layer, drop=drop, share_num=share_num)
