...
class Pretraining(pl.LightningModule):
  ...
  def initialize_imaging_encoder_and_projector(self) -> None:
    ...
    if self.hparams.model.startswith('vit'):
      ...
      if self.hparams.model=='resnet50':
        self.encoder_imaging =nn.Sequential(*list(resnet50().children())[:-2])
      elif self.hparams.model=='resnet18':
        self.encoder_imaging = nn.Sequential(*list(resnet18().children())[:-2])
  ...
  def initialize_tabular_encoder_and_projector(self) -> None:
    ...
    for x in self.field_lengths_tabular:
      ...
      self.con_lengths_tabular.append(x)
    self.encoder_tabular = TabularTransformerEncoder(self.hparams)
  ...
  def initialize_multimodal_encoder_and_predictor(self) -> None:
    ...
    # self.predictor_tabular = TabularPredictor(self.hparams)
  ...
  def forward(self, x: torch.Tensor, tabular: torch.tensor, header: torch.tensor, mask=None) -> torch.Tensor:
    """
    Generates encoding of multimodal data. Pick Clstoken
    """
    _, image_features = self.forward_imaging(x)
    _, tabular_features = self.forward_tabular(tabular, mask=mask, mask_special=mask, header_embedding=header)

    multimodal_features = self.encoder_multimodal(x=tabular_features, image_features=image_features, mask=mask)
    # embeddings = torch.cat([F.adaptive_avg_pool3d(image_features, (1, 1, 1)).flatten(1), tabular_features[:, 0, :]], dim=1)
    #
  ...
  def forward_tabular(self, x: torch.Tensor, mask: torch.Tensor=None, mask_special: torch.Tensor=None, header_embedding: torch.Tensor=None) -> torch.Tensor:
    """
    Generates projection and encoding of tabular data.
    """
    # (B,N,D)
    y = self.encoder_tabular(x, mask=mask, mask_special=mask_special, header_embedding=header_embedding)
    # (B,N1,C) and (B,N2,1)
    z = self.projector_tabular(y[:,0,:])

    # projected feature, original feature
    # return z, y, self.projector_tabular(proto)
    return z, y
  
  def forward_multimodal(self, tabular_features: torch.Tensor, image_features: torch.Tensor, mask=None) -> torch.Tensor:
    """
    Generates prediction of tabular data.
    """
    # y = self.encoder_multimodal(x=tabular_features, image_features=image_features)
    # z = self.predictor_tabular(tabular_features)
    # embeddings = torch.cat([F.adaptive_avg_pool3d(image_features, (1, 1, 1)).flatten(1), tabular_features[:, 0, :]], dim=1)
    y = self.encoder_multimodal(x=tabular_features, image_features=image_features, mask=mask)
    # z = self.(y)
    return y, y[:, 0, :]

    # return z, embeddings
  
  def forward_multimodal_feature(self, tabular_features: torch.Tensor, image_features: torch.Tensor, mask=None) -> torch.Tensor:
    """
  ...
  #
  #   logits, labels = logits[mask].detach(), labels[mask].detach()  #都被mask掉会报错
