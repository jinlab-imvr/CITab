...
class TIPBackbone(nn.Module):
  ...
  def __init__(self, args) -> None:
    ...
    if args.checkpoint:
      ...
      if original_args.model.startswith('vit'):
        ...
        if original_args.model =='resnet50':
          self.encoder_imaging =nn.Sequential(*list(resnet50().children())[:-2])
        elif original_args.model =='resnet18':
      ...
      # self.hidden_dim = args.embedding_dim + args.tabular_embedding_dim
  ...
  def create_tabular_model(self, args):
    ...
    self.encoder_tabular = TabularTransformerEncoder(args)
  ...
  def forward(self, x: torch.Tensor, visualize=False) -> torch.Tensor:
    ...
    if len(x) == 4:
      x_i, x_t, h = x[0], x[1], x[3]
    else:
      x_i, x_t, tab_mask, h = x[0], x[1], x[3], x[4]
      

    x_i = self.encoder_imaging(x_i)  # (B,C,H,W)
    if self.missing_tabular:
      missing_mask = x[2]
      x_t = self.encoder_tabular(x=x_t, mask=missing_mask, mask_special=missing_mask, header_embedding=h)
    else:
      # if len(x) == 5:
      #   x_t = self.encoder_tabular(x_t, mask=tab_mask, mask_special=tab_mask, header_embedding=h)   # (B,N_t,C)
      # else:
      # print(h.shape)
      x_t = self.encoder_tabular(x_t, header_embedding=h)
    ...
    # print(x_m[0].shape, x_m[1].shape, x_m[2].shape)
    # x = self.classifier(x_m[0][:,0,:])
...
if __name__ == "__main__":
  ...
  test_query = torch.randn(2, 5, 32, requires_grad=True)
  test_key = torch.randn(2, 5, 32, requires_grad=True)
  test_value = torch.randn(2, 5, 32, requires_grad=True)
  y = F.scaled_dot_product_attention(test_query, test_key, test_value)