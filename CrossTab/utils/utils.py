...
def create_logdir(name: str, resume_training: bool, wandb_logger):
  ...
  basepath = os.path.join(basepath, 'result')
  ...
  if target.lower() == 'dvm':
    ...
    if augmentation_speedup:
      ...
      transform = tio.Compose([
        tio.Resize(img_size),
        tio.RandomFlip(axes=(0, 1, 2), flip_probability=0.5),  # 3D翻转，类似水平翻转
        tio.RandomAffine(degrees=(0, 45), isotropic=True),
        tio.RandomGamma(log_gamma=(-0.3, 0.3), p=0.5),
        tio.Lambda(convert_to_float)
      ])
