...
class SSLOnlineEvaluator(Callback):  # pragma: no cover
    ...
    def to_device(self, batch: Sequence, device: Union[str, torch.device]) -> Tuple[Tensor, Tensor]:
        ...
        if self.swav:
            ...
            x_i, tv, y, x_orig, x_t_orig, h = batch
            x = x_orig
            x_t = x_t_orig
        else:
            _, x, y = batch
    ...
    def shared_step(
        ...
        with torch.no_grad():
            with set_training(pl_module, False):
                x, y, x_t, h, mask = self.to_device(batch, pl_module.device)
                representations = pl_module(x, tabular=x_t, header=h, mask=mask) if x_t is not None else pl_module(x)
